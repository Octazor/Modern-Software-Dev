...
Tugas ini sebaiknya dieksekusi melalui terminal VS Code Anda:

Buka terminal di VS Code.

Jalankan perintah instalasi: pre-commit install.

Jalankan pengecekan ke semua file: pre-commit run --all-files.

Jika muncul pesan error terkait formatting dari black atau ruff, minta bantuan Copilot dengan prompt:

@workspace Perbaiki semua error formatting dan linting (black/ruff) yang muncul saat saya menjalankan pre-commit. Tunjukkan file mana saja yang perlu disesuaikan.
