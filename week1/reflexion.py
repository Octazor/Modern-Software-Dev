...
# 1. Reflexion System Prompt: Instruct the AI on how to handle the feedback.
YOUR_REFLEXION_PROMPT = """You are an expert Python developer and debugging assistant.
Your previous implementation of the `is_valid_password` function failed several unit tests. 

Your task is to analyze the exact test failures provided by the user, deduce the missing password validation rules, and rewrite the function to fix them.

CRITICAL RULES:
1. Pay close attention to the "Failing checks" in the feedback to deduce the required rules (e.g., minimum length of 8, uppercase, lowercase, digits, no whitespace).
2. The specific special characters required are strictly: !@#$%^&*()-_
3. Output ONLY a single fenced Python code block (```python ... ```) containing the complete, corrected `is_valid_password(password: str) -> bool` function.
4. DO NOT include any conversational text, explanations, or markdown outside the code block.
"""
...
def your_build_reflexion_context(prev_code: str, failures: List[str]) -> str:
    ...
    """
    # 2. Build Context: Format the previous code and the list of errors cleanly 
    # so the model can easily read what went wrong.
    failures_formatted = "\n".join([f"- {failure}" for failure in failures])
    
    return (
        f"Here is your previous code snippet:\n"
        f"```python\n{prev_code}\n```\n\n"
        f"It failed the following tests:\n{failures_formatted}\n\n"
        f"Please rewrite the function to pass all tests based on these failure reasons."
    )
